...
exports.checkWeatherStatusPubSub = functions.pubsub.schedule('every 1 minutes').onRun((context) => {
...
exports.inletStatusUpdated = functions.firestore.document('inlets/{inletId}').onUpdate(async (change, context) => {
  const newValue = change.after.data();
...
exports.checkUploadedImage = functions.storage.object().onFinalize(async (object) => {
    ...
    fs.createReadStream(tempFilePath)
      ...
      .on('end', () => {
        results.forEach(async (result) => {
          const inlet = {};
          const hash = geofire.geohashForLocation([parseFloat(result.latitude), parseFloat(result.longitude)]);
          inlet['geoHash'] = hash;
          inlet['geoLocation'] = new admin.firestore.GeoPoint(parseFloat(result.latitude), parseFloat(result.longitude));
